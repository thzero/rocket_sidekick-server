...
class RocketSetupsService extends AppService {
	...
	async init(injector) {
		...
		this._repositoryRocketSetups = this._injector.getService(AppConstants.InjectorKeys.REPOSITORY_ROCKETSETUPS);
	...
	async delete(correlationId, user, id) {
		...
		try {
			...
			const checklistResponse = await this._serviceChecklists.hasRocketSetup(correlationId, user, id);
			if (this._hasFailed(checklistResponse))
				return checklistResponse;
			// See if its used in a launch
			const launchResponse = await this._serviceLaunches.hasRocketSetup(correlationId, user, id);
	...
	async update(correlationId, user, rocketSetupUpdate) {
		try {
			...
			for (const stage of rocketSetupUpdate.stages)
	...
	async updateMeasurementToMetrics(correlationId) {
		try {
			return await this._repositoryRocketSetups.updateMeasurementToMetrics(correlationId);
		}
		catch (err) {
			return this._error('RocketSetupsService', 'updateMeasurementToMetrics', null, err, null, null, correlationId);
		}
	}
}
